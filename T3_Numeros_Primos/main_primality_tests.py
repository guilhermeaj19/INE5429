...
def solovay_strassen(n: int, t: int = 20) -> bool:
    # Esse teste é importante por conta do cálculo do Simbolo de Jacobi, que demanda um n primo
    if n % 2 == 0: return False # par, exceto o 2, nunca é primo

    # Realiza o teste t vezes de forma a aumentar a sua confiança
    # No caso de 20 tentativas: 1 - (1/2)^k = 0,9999990463257
    for _ in range(t):
...
def miller_rabin(n: int, t: int = 20) -> bool:
    ...
    for _ in range(t):
        ...
        for i in range(k):
            y = pow(a, pow(2,i)*m, n)  # x**(2^i)m (mod n)
            if y == n - 1:
                break
        else:
...
# Recebe o gerador de números e o teste de primalidade escolhido
# Gera números até que o teste de primalidade retorne positivo
def find_prime(rng: RNGAlgorithm, test_primality: "function") -> tuple[int, int, float]:
    ...
    return n, i, (end - start)*1e-6 # Time em ms

# Printa a tabela que é vista no relatório
...
if __name__ == "__main__":
    ...
    for n_bits in lengths:
        ...
        print(f"{n}")
        print(f"It's really prime? {sympy.isprime(n)}") #Debug purpose
